...
export function AdminProductForm({
  ...
  useEffect(() => {
    ...
    if (editingProduct) {
      ...
      loadLinkedCanvaTemplates(editingProduct.id);
    }
  }, [editingProduct]);
  ...
  async function addCanvaTemplate() {
    if (!canvaUrlInput.trim() || !editingProduct?.id) return;
    setAddingCanvaTemplate(true);
    try {
      const res = await fetch("/api/admin/canva-templates/link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          product_id: editingProduct.id,
          canva_template_url: canvaUrlInput.trim(),
        }),
      });
      const data = await res.json();
      ...
      setAddingCanvaTemplate(false);
    }
  }

  async function removeCanvaTemplate(templateId: string) {
    if (!editingProduct?.id) return;
    try {
      const res = await fetch(
        `/api/admin/canva-templates/link?product_id=${editingProduct.id}&template_id=${templateId}`,
        { method: "DELETE" }
      );
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {form.canva_edit_enabled && (
        <div className="rounded-lg bg-surface-container-low p-4 space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {linkedCanvaTemplates.map((tpl) => (
                <div key={tpl.id} className="relative group rounded-lg border border-foreground/10 bg-surface-container overflow-hidden">
                  ...
                  <button
                    type="button"
                    onClick={() => removeCanvaTemplate(tpl.id)}
          ...
          {linkedCanvaTemplates.length === 0 && (
            <p className="text-xs text-on-surface/40 italic">No templates linked yet. Add a Canva template URL above.</p>
          )}
        </div>
      )}
